...
from test.conftest import create_UrlModel_object
...
class TestUrlServices(TestCase):
    ...
    def test_create_response_with_data(self):
        ...
        result = self.services.create_response(HTTPStatus.OK, HTTPStatus.OK.phrase, data_response)
        assert isinstance(result, Response)
        assert result.status is not None
        assert result.data is not None

    def test_create_response_with_errors(self):
        result = self.services.create_response(HTTPStatus.OK, HTTPStatus.OK.phrase, errors="Url Not found")
        assert isinstance(result, Response)
        assert result.status is not None
        assert result.errors is not None

    @patch("shortener.providers.UrlProviders.get_url_providers", return_value=create_UrlModel_object())
    def test_get_url_with_existing_url(self, get_url_response_mock):
        url = 'www.test.com'
        result = self.services.get_url(url=url)
        get_url_response_mock.assert_called_with(
            original_url=url
        )
    ...
    @patch("shortener.providers.UrlProviders.add_url", return_value=create_UrlModel_object())
    @patch("shortener.providers.UrlProviders.get_url_providers", return_value=None)
    def test_get_shortcode_adding_new_url(self, get_url_response_mock, add_url_response_mock):
        url = 'www.test.com'
        result = self.services.get_shortcode(original_url=url)
    ...
    @patch("shortener.providers.UrlProviders.get_url_by_shortcode", return_value=create_UrlModel_object())
    def test_get_url_by_shortcode_with_existing_url(self, get_url_by_shortcode_response_mock):
        shortcode = "test12"
        result = self.services.get_url_by_shortcode(shortcode=shortcode)
        get_url_by_shortcode_response_mock.assert_called_with(
            shortcode=shortcode
        )
        assert result.status == HTTPStatus.OK
        assert result.data is not None
    ...
    def test_get_url_by_shortcode_with_non_existing_url(self, get_url_by_shortcode_response_mock):
        shortcode = "test12"
        result = self.services.get_url_by_shortcode(shortcode=shortcode)
        get_url_by_shortcode_response_mock.assert_called_with(
            shortcode=shortcode
